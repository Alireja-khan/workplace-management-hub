...
if (pStart > -1 && pEnd > -1) {
    ...
    if (midIdx > -1) {
        // extract team block
        const teamBlock = jsxBlock.slice(midIdx + 13, -10).trim(); 
        content = content.slice(0, pStart) + teamBlock + '\n' + content.slice(pEnd);
        console.log("JSX block replaced successfully.");
    }
}

// 7. Inside the now-unified render part, replace references to `filteredTeamProjects` with `currentProjects`, `teamSearchQuery` with `currentSearchQuery`, etc.
...
// IMPORTANT: we must conditionally disable the teamMemberFilter dropdown in Personal Mode.

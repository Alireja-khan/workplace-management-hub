...
export function parseRawSheetText(rawInput, overrideUrl = '') {
  ...
  if (tokens.length >= 13 && tokens.some(t => t.startsWith('FO') || t.startsWith('F0'))) {
    ...
    let assignDate = parseAndFormatDate(tokens[1]) || new Date().toISOString().split('T')[0];
    ...
    // Find status and schedule anywhere in tokens
    tokens.forEach(t => {
    ...
    let teamName = tokens[10] || 'EleSquad';
    let deliveryDate = parseAndFormatDate(tokens[11]) || '';
    let remark = '';

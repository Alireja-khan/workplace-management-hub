...
export function parseRawSheetText(rawInput, overrideUrl = '') {
  ...
  if (tokens.length >= 13 && tokens.some(t => t.startsWith('FO') || t.startsWith('F0'))) {
    ...
    let grossAmount = tokens.length > 5 && tokens[5].startsWith('$') ? String(parseCurrency(tokens[5]) || '') : '';
    let payoutPercentage = tokens.length > 12 && tokens[12].startsWith('$') ? String(parseCurrency(tokens[12]) || '') : '';

import { MONTH_LIST, getMonthFromDate } from './dateUtils.js';
...
export function parseRawSheetText(rawInput, overrideUrl = '') {
  ...
  // Tokenize by tabs, newlines, or spaces
  let tokens = [];
  if (raw.includes('\t')) {
    ...
    tokens = raw.split(/\s+/).map((t) => t.trim()).filter((t) => t.length > 0);
  }
  ...
  // If we have exactly 13 tokens, this perfectly maps to the Team Orders format
  if (tokens.length >= 13 && tokens.some(t => t.startsWith('FO') || t.startsWith('F0'))) {
    // 0: Sales Person, 1: Date, 2: Profile, 3: Client ID, 4: Order #
    // 5: Gross Amount, 6: Assigned Name, 7: Net Amount, 8: Order Status
    // 9: Sheet Link, 10: Team Name, 11: Deli Date, 12: Percentage (Payout)
    let salesPerson = tokens[0];
    // Dynamically find dates in any column
    let allDates = tokens
      .filter(t => /\d{1,4}[-/\s][A-Za-z0-9]{2,4}[-/\s]\d{2,4}/.test(t) || /\d{1,2}\/\d{1,2}\/\d{4}/.test(t))
      .map(t => parseAndFormatDate(t))
      .filter(Boolean);
      
    let assignDate = allDates[0] || new Date().toISOString().split('T')[0];
    let deadline = allDates.length > 1 ? allDates[allDates.length - 1] : '';
    let estimatedDeliveryDate = allDates.length > 2 ? allDates[1] : '';
    let deliveryDate = '';

    let profileName = tokens[2];
    let clientUserId = tokens[3];
    let orderNumber = tokens.find(t => t.startsWith('FO') || t.startsWith('F0')) || tokens[4];
    
    // Find the amounts using regex matching currency tokens
    let grossAmount = '';
    ...
    let assignedMembers = [];
    if (tokens[6] && !tokens[6].startsWith('$')) {
      assignedMembers = tokens[6].split(/[,/]/).map(s => s.trim()).filter(Boolean);
    }
    ...
    // Find status, schedule, and team anywhere in tokens
    tokens.forEach(t => {
      ...
      else if (lower === 'nra') orderStatus = 'NRA';
      else if (lower.includes('need requirement')) orderStatus = 'Need Requirements';
      else if (lower.includes('issue')) orderStatus = 'Issue';
      
      if (lower.includes('repeat')) timeSchedule = 'Repeat Order';
      else if (lower.includes('add-on') || lower.includes('addon')) timeSchedule = 'Add-on';
      else if (lower.includes('fresh')) timeSchedule = 'Fresh Query';
      
      if (lower.includes('elesquad') || lower.includes('smt')) teamName = t;
    });

    let rawSheetLink = tokens.find(t => t.includes('http') || t.includes('docs.google.com') || t.startsWith('[')) || tokens[11] || '';
    if (rawSheetLink.startsWith('[')) {
      rawSheetLink = extractUrlFromText(rawSheetLink);
    }
    const validSheetUrl = ensureValidUrl(rawSheetLink) || ensureValidUrl(extractedUrl);
  ...
  // Fallback Heuristic Approach for <13 or >13 tokens
  ...
  tokens.forEach((t) => {
    ...
    // Check if Order Number (e.g. FO2231D9D1308 or F02231...)
    if (/^F[O0][A-Z0-9]+$/i.test(t)) {
      orderNumber = t;
      return;
    }
    ...
    if (lower.includes('tamim') || lower.includes('shuvo') || lower.includes('rone') || lower.includes('chayon') || lower.includes('c_forward') || lower.includes('sales')) {
      salesPerson = t;
      return;
    }
...
export function parsePersonalSheetText(rawInput, overrideUrl = '') {
  ...
  const teamParsed = parseRawSheetText(rawInput, overrideUrl);
  if (teamParsed) {
    return {
      salesPerson: teamParsed.salesPerson || '',
      assignDate: teamParsed.assignDate || new Date().toISOString().split('T')[0],
      month: teamParsed.month || MONTH_LIST[new Date().getMonth()],
      profileName: teamParsed.profileName || '',
      clientUsername: teamParsed.clientUserId || '',
      orderNumber: teamParsed.orderNumber || '',
      amount: teamParsed.amount || '',
      estimatedDeliveryDate: teamParsed.estimatedDeliveryDate || '',
      deliveryDate: teamParsed.deliveryDate || '',
      remark: teamParsed.remark || '',
      orderStatus: teamParsed.orderStatus || 'Wip',
      instructionSheet: teamParsed.sheetLink || '',
      percentage: teamParsed.percentage || '',
      notes: teamParsed.note || teamParsed.remark || '',
      timeSchedule: teamParsed.timeSchedule || 'Fresh Query',
      deadline: teamParsed.estimatedDeliveryDate || teamParsed.deliveryDate || '',
    };
  }
